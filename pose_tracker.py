...
功能说明（本版本核心变化）：
    ...
    教学路径与交互界面规范"来区分：
        ...
        C组（常规对照）：右侧只显示"干净"的原始视频画面，不做任何姿态检测、
                        不画骨骼线、不显示颜色框、也不显示任何文字。
...
class VideoWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # ---------------- 2. 只有 A/B 组需要用到 MediaPipe 姿态检测器 ----------------
            # C组是"干净对照组"，完全不需要姿态检测，跳过初始化以节省资源
            need_pose_detection = self.group in ("A", "B")
